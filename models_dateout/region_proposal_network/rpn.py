...
class ProposalCreator():
    ...
    def __call__(self, loc, score, anchor, img_size, scale=1.):
        ...
        @param score:
        @param anchor:
        @param img_size: 图像size, 防止建议框超出图像边缘
        @param scale: 缩放
        @return:
        """
...
class RegionProposalNetwork(nn.Module):
    ...
    def __init__(self, in_channels=640, mid_channels=640, ratios=[0.5, 1, 2], anchor_scales=[8, 16, 32], feat_stride=16,
                 mode="training", nms_iou=0.7,
                 n_train_pre_nms=12000,
                 n_train_post_nms=600,
                 n_test_pre_nms=3000,
                 n_test_post_nms=300,
        ...
        #   分类预测先验框内部是否包含物体
        # -----------------------------------------#
        self.score = nn.Conv2d(mid_channels, out_channels=n_anchor * 2, kernel_size=1, stride=1, padding=0)
        # -----------------------------------------#
        #   回归预测对先验框进行调整
        # -----------------------------------------#
        ...
        #   用于对建议框解码并进行非极大抑制
        # -----------------------------------------#
        self.proposal_layer = ProposalCreator(mode, nms_iou=nms_iou,
    ...
    def forward(self, x, img_size, scale=1.):
        r"""
        RPN的前向传播, 输入size: (batch, channel, h, w),

        * 一个锚点产生的锚框数量: k

        * 产生锚框数量: all_anchor_num = h * w * k

        * 筛选后的rp数量: final_rpn_num
