...
if __name__ == '__main__':
    ...
    # ----------------------------------------------------------------------------------------
    weight_path = 'weights_coco_r50/frnod999_999.pth'
    ...
    for i in range(mission):
        ...
        for index in pbar:
            ...
            savePath = os.path.join(root, 'predictions_r50', fsod.coco.loadImgs(target['image_id'])[0]['file_name'])
    ...
    with open('datasets/coco/annotations/coco_prediction_r50.json', 'w') as f:
        json.dump(eval_results, f)
    # 验证
    gt_path = 'datasets/coco/annotations/instances_train2017.json'  # 存放真实标签的路径
    dt_path = "datasets/coco/annotations/coco_prediction_r50.json"  # 存放检测结果的路径
    cocoGt = COCO(gt_path)
