...
if __name__ == '__main__':
    ...
    if not (os.path.exists('weights_fsod_msf/results')):
        os.makedirs('weights_fsod_msf/results')
    ...
    # ----------------------------------------
    if not os.path.exists('weights_fsod_msf/results/loss_per_image.json'):
        with open('weights_fsod_msf/results/loss_per_image.json', 'w') as f:
            json.dump({}, f)
            print('创建loss_per_image.json')
    if not os.path.exists('weights_fsod_msf/results/loss_per_image_val.json'):
        with open('weights_fsod_msf/results/loss_per_image_val.json', 'w') as f:
            json.dump({}, f)
            print('创建loss_per_image_val.json')

    # ----------------------------------------------------------------------------------------
    continue_weight = 'weights_fsod_msf/frnod4_90.pth'
    ...
    for e in range(0, epoch):  # 4 * 60000 * 8
        ...
        if e < 15:
            ...
            optimizer = torch.optim.SGD(model.parameters(), lr=0.00004, momentum=0.9, weight_decay=0.0005)
        # if e < 20:
        ...
        for i in range(num_mission):
            ...
            # print('将本次任务的loss写入weights_fsod_msf/results/loss_per_image.json')
            with open('weights_fsod_msf/results/loss_per_image.json', 'r') as f:
                content = json.load(f)
            with open('weights_fsod_msf/results/loss_per_image.json', 'w') as f:
            ...
            if (i + 1) % 10 == 0:
                print('将本次任务的权重写入weights_fsod_msf/frnod{}_{}.pth'.format(e + 1, i + 1))
                torch.save({'models': model.state_dict()}, 'weights_fsod_msf/frnod{}_{}.pth'.format(e + 1, i + 1))
            ...
            # print('将本次任务的loss写入weights_fsod_msf/results/loss_per_image_val.json')
            with open('weights_fsod_msf/results/loss_per_image_val.json', 'r') as f:
                content = json.load(f)
            with open('weights_fsod_msf/results/loss_per_image_val.json', 'w') as f:
        ...
        with open('weights_fsod_msf/results/loss_bepoch{}.json'.format(e + 1), 'w') as f:
